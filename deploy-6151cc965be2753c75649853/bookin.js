...
const nav = document.querySelector('.methods ul');
const childrenLI = document.querySelectorAll('.methods ul li');
burger.addEventListener('click', () => {
  nav.classList.toggle('nav-active');
  childrenLI.forEach((link, index) => {
    if (link.style.animation) {
      link.style.animation = '';
    }else{
      link.style.animation = `navlinkFade 1s ease ${index / 5 + 0.1}s alternate forwards `;
    }
  });
